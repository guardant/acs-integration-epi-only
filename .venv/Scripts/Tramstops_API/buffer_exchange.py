...
class buffer_exchange:
    ...
    def buffer_exchange_workflow(self,logger,number_of_batches):
        ...
        helper.abort_instrument_job_for_started_state(self, logger, config_values.host, config_values.username,
                                                      config_values.password, config_values.instrument_serial_number)
        ...
        barcodes_usb_plate_id = helper.get_plate_from_labware(self, logger, config_values.host, config_values.username,
                                                              config_values.password, variables.get('buffer_exchange_be_spri_plate_lot_number'))
        barcodes_universal_te_plate_id = helper.get_plate_from_labware(self, logger, config_values.host,config_values.username,config_values.password,
        ...
        if number_of_batches == "1":
            ...
            time.sleep(5)
            hamilton_ui.enter_simulated_barcodes(self,logger,config_values.window_name_pltCar2, texts_to_enter=[config_values.eto_plate, barcodes_usb_plate_id[0],
                                                                 DWT_plate_id1, config_values.nocode, ple_plate_id])
            time.sleep(30)
            hamilton_ui.enter_simulated_barcodes(self,logger,config_values.window_name_pltCar1, texts_to_enter=[config_values.nocode,
            ...
            time.sleep(5)
            hamilton_ui.enter_simulated_barcodes(self,logger,config_values.window_name_pltCar3, texts_to_enter=[config_values.nocode, barcodes_usb_plate_id[0],
                                                                 DWT_plate_id1, config_values.nocode, ple_plate_id])
            random_number = random.randint(10000, 99999)
            DWT_plate_id_2 = config_values.dwt_plate_id + str(random_number)
            ple_plate_id_2 = helper.create_ple_plate_id(self, logger)
            hamilton_ui.enter_simulated_barcodes(self,logger,config_values.window_name_pltCar2,texts_to_enter=[config_values.eto_plate, barcodes_usb_plate_id[1],
                                                                 DWT_plate_id_2, config_values.nocode, ple_plate_id_2])
            output_plates = helper.get_ex_elution_plates(self, logger, config_values.host, config_values.username,config_values.password, 'cfDNA Extraction Upload')
            hamilton_ui.enter_simulated_barcodes(self,logger, config_values.window_name_pltCar1,texts_to_enter=[config_values.nocode,barcodes_universal_te_plate_id[0],
                                                                   output_plates[0], output_plates[1], config_values.nocode])
