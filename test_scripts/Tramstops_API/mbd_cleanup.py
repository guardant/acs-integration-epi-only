...
class mbd_cleanup:
    ...
    def mbd_cleanup_workflow(self,logger,number_of_batches):
        ...
        variables_from_response = helper.process_samples_new(self,logger,config_values.host, config_values.username, config_values.password,
                                               config_values.stop_at_mbd_cleanup, archive_path)
        ...
        time.sleep(20)
        hamilton_ui.select_options_from_dialog_window(self,["No Wait","No Pipetting","Stoppable Timers"])
        ...
        hamilton_ui.enter_value_on_bulk_reagent_scan_window(self, variables.get('mbd_cleanup_etoh_lot_number'))
        time.sleep(3)
        barcodes_msb_plate_id = helper.get_plate_from_labware(self, logger, config_values.host, config_values.username,
                                                              config_values.password,
                                                              variables.get('mbd_cleanup_mbdc_spri_1_lot_number'))
        barcodes_dsb_plate_id = helper.get_plate_from_labware(self, logger, config_values.host, config_values.username,
                                                              config_values.password,
                                                              variables.get('mbd_cleanup_mbd_dsb_lot_number'))
        time.sleep(3)
        if number_of_batches == "1":
            hamilton_ui.clicks_on_deck_setup_window(self, config_values.epionly_mbd_cleanup_load1_deck_setup)
            hamilton_ui.enter_simulated_barcodes(self, logger, config_values.window_name_pltCar3,texts_to_enter=[ barcodes_msb_plate_id[0],config_values.nocode,
                                                                config_values.nocode,config_values.nocode, barcodes_dsb_plate_id[0]])
            random_number = random.randint(10000, 99999)
            DWE_plate_id1 = config_values.dwe_plate_id + str(random_number)
            barcode_dwt_plate = helper.get_plates_for_completed_instrument_job(self, logger, config_values.host,config_values.username, config_values.password,'MBDW Hyper Plate')
            hamilton_ui.enter_simulated_barcodes(self, logger, config_values.window_name_pltCar2,texts_to_enter=[barcode_dwt_plate[0], config_values.nocode,
                                                                 config_values.eto_plate, ple_plate_id, config_values.nocode])
        else:
            print("abs")
